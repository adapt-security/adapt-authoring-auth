...
class AuthToken {
  ...
  static async initRequestData(req) {
    ...
    const [auth, users] = await App.instance.waitForModule('auth', 'users');
  ...
  static async generate(authType, userData, options = {}) {
    ...
    auth.log('debug', 'AUTH_TOKEN_ISSUED', data.userId, data.authType, expiresIn);
    return token;
  }
  /**
   * Decodes a token
  ...
  static async revoke(query) {
    ...
    try {
      ...
      results.forEach(e => auth.log('debug', 'AUTH_TOKEN_REVOKED', r.userId, r.authType))
    } catch(e) {
