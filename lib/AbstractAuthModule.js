...
class AbstractAuthModule extends AbstractModule {
  ...
  async init() {
    ...
    if(this.getConfig('secureTokenGeneration')) {
      this.secureRoute(`/generatetoken`, 'post', ['generatetoken:auth']);
    } else {
      this.unsecureRoute('/generatetoken', 'post');
    }
  ...
  async authenticateHandler(req, res, next) {
    ...
    try {
      ...
      res.sendError(this.app.errors.UNAUTHENTICATED);
    }
  }
  /**
  ...
  async generateTokenHandler(req, res, next) {
    this.authenticateHandler(req, res, e => {
      if(e) return next(e);
      res.json({ token: req.auth.token });
    });
  }
  /**
