...
class AbstractAuthModule extends AbstractModule {
  ...
  async init() {
    ...
    this.router.addRoute({
      ...
      route: '/tokens',
      handlers: { post: this.retrieveTokensHandler.bind(this) }
    }, {
      route: '/register',
      handlers: { post: this.registerHandler.bind(this) }
    }, ...this.routes);

    this.secureRoute(`/generatetoken`, 'post', ['generatetoken:auth']);
    this.secureRoute(`/tokens`, 'get', ['read:me']);
   ...
   */
   async generateTokenHandler(req, res, next) {
    try {
      res.json({ token: await AuthToken.generate(this.type, { _id: req.auth.user._id }, { lifespan: req.body.lifespan }) });
    } catch(e) {
      return next(e);
    }
  }
   ...
   * @param {Function} next
   */
   async retrieveTokenHandler(req, res, next) {
     try {
