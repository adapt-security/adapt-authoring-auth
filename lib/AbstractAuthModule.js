...
class AbstractAuthModule extends AbstractModule {
  ...
  async init () {
     ...
     */
    this.users = users
    const config = await loadRouteConfig(this.rootDir, this, { schema: 'authroutes' })
    if (config) {
    ...
    const routes = config
      ? config.routes
      : [
          { route: '/', handlers: { post: this.authenticateHandler.bind(this) }, permissions: { post: null } },
          { route: '/register', handlers: { post: this.registerHandler.bind(this) }, permissions: { post: ['register:users'] } },
          { route: '/enable', handlers: { post: this.enableHandler.bind(this) }, meta: apidefs.enable, permissions: { post: ['write:users'] } },
          { route: '/disable', handlers: { post: this.enableHandler.bind(this) }, meta: apidefs.disable, permissions: { post: ['write:users'] } },
          ...(this.routes || [])
        ]
