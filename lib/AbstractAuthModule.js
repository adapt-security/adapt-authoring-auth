...
class AbstractAuthModule extends AbstractModule {
  ...
  async init () {
    ...
    if (!this.routes) {
      this.routes = await this._getDefaultRoutes()
    }
    this._applyRoutes(this.routes)
    /**
  ...
  async setValues () {
    ...
    if (config) {
      ...
      await this._applyRouteConfig(config)
    }
  }
   ...
   */
  async _applyRouteConfig (config) {
    const defaultRoutes = await this._getDefaultRoutes()
    const customRoutes = config.routes || []
    this.routes = [...defaultRoutes, ...customRoutes]
  }
   ...
   */
  async _getDefaultRoutes () {
    const template = await readJson(new URL('./default-routes.json', import.meta.url).pathname)
