...
class Authentication {
   ...
   * @param {String} authType The type of authentication to disavow. If no authType is specified, all auth tokens will be revoked
   * @return {Promise}
   */
  async disavowUser(userId, authType) {
    if(!userId) {
      throw App.instance.errors.INVALID_PARAMS;
    }
    if(!authType) {
      const users = await App.instance.waitForModule('users');
      const { authTypes } = await users.find({ _id: userId });
      return Promise.all(authTypes.map(type => this.disavowUser(userId, type)));
    }
  ...
  async disavowHandler(req, res, next) {
    try {
      ...
      AuthUtils.log('debug', 'USER_LOGOUT', req.auth.token.type, req.auth.user._id.toString());
    } catch(e) {
      return next(e);
    }
    res.status(204).end();
