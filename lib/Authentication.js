...
class Authentication {
   ...
   * @param {external:express~Request} req _id of the user to disavow
   * @param {Object} options
   * @param {boolean} options.removeAll Whether to revoke all tokens. If false/undefined, only the token linked to the current request will be revoked.
   * @return {Promise}
   */
  async disavowUser(req, options = {}) {
    const users = await App.instance.waitForModule('users');
    await users.find({ _id: req.auth.user._id });
    return AuthToken.revoke(options.removeAll ? { userId } : { signature: req.auth.token.signature });
  }
  /**
