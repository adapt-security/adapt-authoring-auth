...
class AuthUtils {
  ...
  */
  ...
  static async generateToken(data) {
    const token = await jwtSignPromise(data, this.getConfig('secret'), { expiresIn: this.getConfig('tokenLifespan'), issuer: this.getConfig('tokenIssuer') });
  ...
  static async decodeToken(token, secret = this.getConfig('secret')) {
    let tokenData;
    try {
      tokenData = await jwtVerifyPromise(token, secret);
    } catch(e) {
