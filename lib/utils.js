  ...
  */
  static authenticate(req) {
    return new Promise((resolve, reject) => {
      ...
      */
      App.instance.logger.log('warn', 'auth-utils', `Using local reqScopes.json for ${req.method} ${req.originalUrl}`);
      const scopes = require('fs').readFile(require('path').join(process.cwd(), 'reqScopes.json'), (error, contents) => {
        if(error) {
          return reject('Failed to load reqScopes.json');
        }
        try {
          req.auth = {
            id: { userId: Date.now().toString().padStart(24, '0') },
            scopes: JSON.parse(contents)
          };
          resolve();
        } catch(e) {
          reject('Failed to parse reqScopes.json');
        }
      });
    });
  }
