...
class Authentication {
  ...
  */
  ...
  async disavowUser(authType, _id) {
    if(!authType || !_id) {
      throw new Error('Must provide authType and user _id');
    }
    return AuthToken.revoke({ _id, authType });
  }
  /**
  * Verifies the incoming request is correctly authenticated
  * @param {ClientRequest} req
  ...
  */
  async disavowHandler(req, res, next) {
    ...
    try {
      try { 
        return AuthToken.revoke({ _id: req.auth.token.jti });
      } catch(e) {
        next(e);
      }
    }
