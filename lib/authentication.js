...
const { App, Hook } = require('adapt-authoring-core');
...
class Authentication {
  ...
  */
  ...
  /**
  * Revokes current authentication for the authenticated user
  * @param {ClientRequest} req The incoming request object
  * @return {Promise}
  */
  async disavow(req) {
    await this.disavowHook.invoke(req);
    return AuthToken.revoke(req.auth.token.jti);
  }
  /**
  ...
  */
  async disavowHandler(req, res, next) {
    try {
      if(req.auth) await this.disavow(req);
