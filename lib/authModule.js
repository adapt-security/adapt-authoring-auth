  ...
  */
  ...
  authenticateHandler() {
    return async (req, res, next) => {
      ...
      try {
        res.json({ token: await this.authenticate({ type, value }) });
      } catch(e) {
        next(e);
      }
    };
  ...
  async authenticate(data) {
    ...
    const [user] = await this.users.find({ email: authData.email });
    const scopes = [];
    ...
    if(user.responsibilities.length) {
      const rFilter = { $or: user.responsibilities.map(r => Object.assign({}, { _id: r._id })) };
      const responsibilities = await this.responsibilities.find(rFilter);
    ...
    return new Promise(async (resolve, reject) => {
      jwt.sign(data, this.getConfig('secret'), { issuer: 'test' }, (error, token) => {
        ...
        this.app.waitForModule('mongodb').then(db => {
          ...
          db.insert('authtokens', data).then(() => resolve(token), reject);
        }, reject)
      });
    });
  }
