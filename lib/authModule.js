...
class AuthModule extends AbstractModule {
  ...
  async init() {
    ...
    server.api.addMiddleware(this.handleRequest.bind(this));
  ...
  */
  async handleRequest(req, res, next) {
    ...
    if(this.routes.unsecured[req.method.toLowerCase()][`${req.baseUrl}${req.path}`]) {
      return next(); // route is unsecured
    }
    if(!req.auth.header) {
