...
class AuthModule extends AbstractModule {
  ...
  async init() {
    const [ server, responsibilities, users ] = await this.app.waitForModule('server', 'responsibilities', 'users');
    server.api.addRoute(
  ...
  */
  ...
  authenticate() {
    return async (req, res, next) => {
      try {
        const authData = await Promise.race(this.authenticators.map(a => a(req)));

        console.log(authData.userId);

        let user = await this.users.find(this.users.schemaName, this.users.collectionName, { _id: authData.userId });

        if(!user) {
          user = await this.registerUser(authData);
        }
        const responsibilities = await this.responsibilities.find(this.responsibilities.schemaName, this.responsibilities.collectionName, { _id: { $or: user.responsibilities } });
        const scopes = responsibilities.reduce((memo, r) => {
          memo.push(...r.scopes);
          return memo;
        }, []);
        res.json({ token: await this.generateToken({ userId: user._id, scopes }) });
      } catch(e) {
        next(e);
      }
    };
  ...
  registerUser() {
    return async (req, res, next) => {
      const user = await this.users.find(this.users.schemaName, this.users.collectionName, { email: req.body.email });
      console.log(user);
    };
  }
  generateTokenForUser(userId) {
    return new Promise(async (resolve, reject) => {
      const user = await this.users.find(this.users.schemaName, this.users.collectionName, { _id: userId });
      ...
      const tokenData = {
        _id: user._id,
        scopes: responsibilities.reduce((memo, r) => {
          memo.push(...r.scopes);
          return memo;
        }, [])
      };
